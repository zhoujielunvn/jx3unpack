...
function SetBagPendentBoxSize(nNewBoxSize)
	local player = GetClientPlayer()
	if not player then
		return
	end
	
	player.SetBagPendentBoxSize(nNewBoxSize)
end

function SetPendentBoxSize(dwPendentType, nNewBoxSize)
	local player = GetClientPlayer()
	if not player then
		return
	end
	
	player.SetPendentBoxSize(dwPendentType, nNewBoxSize)
end
...
function OnSyncAccountSafe(nSprintPowerMax, nHorseSprintPowerMax, nSprintPowerRevive, nMaxStamina, nSprintPower, nHorseSprintPower)
    local player = GetClientPlayer();
    if not player then
    	return
    end
    	
    player.nSprintPowerMax = player.nSprintPowerMax + nSprintPowerMax
    player.nHorseSprintPowerMax = player.nHorseSprintPowerMax + nHorseSprintPowerMax
    player.nSprintPowerRevive = player.nSprintPowerRevive + nSprintPowerRevive
    player.nMaxStamina = player.nMaxStamina + nMaxStamina
	player.nSprintPower = nSprintPower
	player.nHorseSprintPower = nHorseSprintPower
end
